...
EOF
            ...
            if [ -n "$SECTION" ]; then
                KEY=$(printf '%s\n' "$UCI_WIRELESS" | awk -F= -v sect="${SECTION}.key=" 'index($0, sect)==1 {gsub(/'"'"'/, "", $2); print $2; exit}')
                ENCRYPTION=$(printf '%s\n' "$UCI_WIRELESS" | awk -F= -v sect="${SECTION}.encryption=" 'index($0, sect)==1 {gsub(/'"'"'/, "", $2); print $2; exit}')
...
EOF
            ...
            if [ -n "$SECTION" ]; then
                KEY=$(printf '%s\n' "$UCI_WIRELESS" | awk -F= -v sect="${SECTION}.key=" 'index($0, sect)==1 {gsub(/'"'"'/, "", $2); print $2; exit}')
                OCV=$(printf '%s\n' "$UCI_WIRELESS" | awk -F= -v sect="${SECTION}.ocv=" 'index($0, sect)==1 {gsub(/'"'"'/, "", $2); print $2; exit}')
                [ -z "$OCV" ] && OCV="0"

                if [ "$COUNTRY" = "World" ]; then
                    WIFI_DEV=$(printf '%s\n' "$UCI_WIRELESS" | awk -F= -v sect="${SECTION}.device=" 'index($0, sect)==1 {gsub(/'"'"'/, "", $2); print $2; exit}')
                    [ -n "$WIFI_DEV" ] && COUNTRY_UCI=$(printf '%s\n' "$UCI_WIRELESS" | awk -F= -v sect="wireless.${WIFI_DEV}.country=" 'index($0, sect)==1 {gsub(/'"'"'/, "", $2); print $2; exit}')
